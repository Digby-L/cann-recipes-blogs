...
**需求：** 使用文档目录结构在全站范围内筛选内容，替代从文件名拆解关键词的 Tags 分类。

**实现方案：**
- `build_content.py` 不再根据文件名生成 `tags` 字段。
- 顶部搜索框旁提供内容范围筛选入口，支持多选 Infer、Train、Embodied AI 和 CANN Features。
- 选中一个或多个大类后，仅展示这些大类下的子类选项；例如 Infer 展示 LLM、Multimodal、Recommendation，Train 展示 Pretrain、RL。
- 子类筛选仅约束所属大类；同时选择多个大类时，未选择子类的大类仍展示其全部文档。
- 在大类/子类筛选下方以横线分隔技术属性筛选：`quantization`、`parallelism`、`operator`、`cannFeatures`、`hardware`、`frameworks`。
- 每个技术属性支持多选；同一属性内任一值命中即可，不同技术属性之间必须同时命中。大类/子类仍作为内容范围约束，多个已选大类取并集。`none` 是可选择的空值，用于查找未涉及该属性的文档。
- 大类/子类筛选可与全文关键词组合使用，结果页显示当前筛选范围并支持一键清除。
- 报告卡片和搜索结果不再展示或点击旧 Tags 标签。

**修改文件：** `build_content.py`（移除文件名标签推导）、`index.html`（范围筛选和搜索逻辑）

**验证方法：**
1. 运行 `python build_content.py`，确认 `content/index.json` 的报告条目不包含 `tags`。
2. 选择 Infer，确认仅显示 Infer 文档，并出现其子类选项。
3. 在 Infer 与 Train 同时选中时，确认两个大类的文档均显示。
4. 在 Infer 下选择 LLM 时，确认只限制 Infer 为 LLM，Train 仍显示全部已选内容。
5. 在任意技术属性中多选两个值，确认结果命中其中任一值；再选择另一属性，确认结果同时满足两个属性。
6. 选择任意属性的 `none`，确认结果只包含该属性无适用枚举的文档。
7. 输入关键词后选择内容范围，确认结果同时满足全文搜索和范围筛选。
...
├── report_entry() — 生成静态字段、封面 URL、来源仓库等元数据
...
2. 打开 Category 视图，确认含图片的报告卡片使用了报告中的真实图  片
...
**需求：** 界面文案支持中文/英文切换，并为同一篇报告的英文正文预留独立内容路径。

**方案设计：**
- 定义 `i18n = { en: {...}, zh: {...} }` 翻译字典
- Header 区域添加语言切换按钮（中文 / EN）
- 切换时批量替换 DOM 中所有 UI 文案（按钮、placeholder、标签、提示语）
- 中文报告位于原目录，英文报告位于同级 `en/` 目录并保持相同文件名
- 英文报告存在时同步切换标题、侧栏标题、全文索引和正文；不存在时保留中文正文并显示提示
- 用户选择持久化到 `localStorage`

**修改文件：** `index.html`、`build_content.py`、`content/index.json`、`content/search-index.json`

**验证方法：**
1. 点击语言切换按钮 EN → CN，确认所有 UI 文案变为中文（按钮、搜索 placeholder、分类标签等）
2. 再次点击 CN → EN，确认全部恢复英文
3. 刷新页面，确认语言选择被持久化
4. 打开只有中文版本的报告，切换英文后确认显示缺失提示并继续展示中文正文
5. 在同级 `en/` 目录增加同名英文报告，重新构建后确认标题、搜索索引和正文可切换
6. 检查切换后是否有文案遗漏（侧边栏、footer、浮动按钮提示）
...
|------|------|
| **目标** | 按内容大类和子类筛选报告 |
| **验收标准** | ① 支持多选四个内容大类 ② 选中大类后展示其子类 ③ 支持多大类和子类组合 ④ 支持与关键词组合筛选 |
| **实现要点** | 目录层级是唯一分类数据源，不从文件名生成 Tags |
| **修改文件** | `build_content.py`（移除 tags）、`index.html`（范围筛选和搜索逻辑） |
