...
- 不再根据文件名拆解关键词或生成 Tags 分类。
- 搜索框旁提供内容范围筛选，支持多选 Infer、Train、Embodied AI 和 CANN Features。
- 选中大类后动态展示该大类的子类，并支持多个大类与子类的组合筛选。
- 筛选面板以横线分隔技术属性区域，支持按量化、并行切分、算子方式、CANN 特性、硬件和框架组合筛选。
- 所有技术属性均支持多选；同一属性内取并集，不同技术属性之间取交集，并提供 `none` 空值选项。
- 支持内容范围与全文关键词组合筛选。
- 结果页显示当前内容范围，并支持一键清除。
...
- Header 支持中文与英文 UI 切换，并将语言选择持久化到浏览器。
- 英文文档使用中文文档同级 `en/` 目录下的同名 Markdown 文件。
- 英文文档存在时同步切换标题、侧栏短标题、全文搜索内容和报告正文。
- 英文文档尚未提供时保留中文正文，并显示明确的版本缺失提示。
- 中英文正文使用独立静态缓存，避免同名文件相互覆盖。

## 构建与部署

- manifest 生成器只依赖 Python 标准库。
- 支持动态发现 GitCode 目录结构。
- manifest 包含文件路径、目录层级、静态字段、封面和来源仓库路由元数据。
