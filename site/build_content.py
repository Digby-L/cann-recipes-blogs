#!/usr/bin/env python3
"""
Build a lightweight report metadata manifest from the local docs repository.

The website lives in the same repository as the Markdown reports, so the
browser reads report bodies and adjacent images directly from the checked-in
documentation paths. This script only generates content/index.json and
content/search-index.json for navigation and full-text search.
...
DOCS_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, os.pardir))
DOCS_SNAPSHOT_DIR = (
    os.path.abspath(os.environ["DOCS_SNAPSHOT_DIR"])
    if os.environ.get("DOCS_SNAPSHOT_DIR")
    else DOCS_ROOT
)
REPO_SNAPSHOT_DIRS = {
...
REPO_SNAPSHOT_DIRS["cann-recipes-docs"] = DOCS_SNAPSHOT_DIR
...
SOURCE_REPOS = {
    "Infer": ("cann-recipes-docs", "https://gitcode.com/tian-ccs/cann-recipes-docs"),
    "Train": ("cann-recipes-docs", "https://gitcode.com/tian-ccs/cann-recipes-docs"),
    "Embodie_AI": ("cann-recipes-docs", "https://gitcode.com/tian-ccs/cann-recipes-docs"),
...
def metadata_enum_manifest():
    """Export complete metadata enum values for the client-side filter UI."""
    if DOCS_ROOT not in sys.path:
        sys.path.insert(0, DOCS_ROOT)
    try:
        from metadata_enums import (  # pylint: disable=import-outside-toplevel
            CannFeature,
            Framework,
            Hardware,
            LlmSpeculativeInference,
        ...
        print(f"Warning: failed to load metadata enums: {error}", file=sys.stderr)
        return {}
    ...
    enum_fields = {
        ...
        "operator": Operator,
        "cannFeatures": CannFeature,
        "hardware": Hardware,
        "frameworks": Framework,
        "llmSpeculativeInference": LlmSpeculativeInference,
        "multimodalDitCache": MultimodalDitCache,
    }
    return {
        field: [item.value for item in enum_class]
        for field, enum_class in enum_fields.items()
    }


def metadata_label_manifest():
    """Export localized metadata labels for the client-side filter UI."""
    if DOCS_ROOT not in sys.path:
        sys.path.insert(0, DOCS_ROOT)
    try:
        from metadata_labels import metadata_label_manifest as labels  # pylint: disable=import-outside-toplevel
    except Exception as error:
        print(f"Warning: failed to load metadata labels: {error}", file=sys.stderr)
...
def report_entry(category, model_key, repo, branch, doc_dir, report_file):
    ...
    if image_src:
        if image_src in images:
            ...
            cover_image = (
                resolved
                if repo == "cann-recipes-docs"
                else f"{GITCODE_RAW_CDN}/{repo}/raw/{branch}/{resolved}"
            )
...
def main():
    ...
    manifest = {
        "_metadataEnums": metadata_enum_manifest(),
        "_metadataLabels": metadata_label_manifest(),
    }
