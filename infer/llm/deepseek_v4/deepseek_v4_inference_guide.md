...
## 量化策略
### Atlas-A3 Pod
#### Int8 (推荐DeepSeekV4-Flash模型使用)

基于原生Hybrid FP8-MXFP4权重，本实践支持了Int8 W8A8量化。相对于BF16推理，W8A8量化可以有效降低端到端时延，提升系统吞吐。量化策略如下：
...
#### Hybrid INT8-INT4 (推荐DeepSeekV4-Pro模型使用)

由于DeepSeekV4-Pro模型的参数量较大，MoE部分使用W8A8推理会面临较大的内存和访存瓶颈。本实践针对`DeepSeekV4-Pro`模型设计了Hybrid INT8-INT4的量化。整体量化策略同上，主要对`MoE`部分采用INT4量化，相比较纯INT8量化策略，Hybrid INT8-INT4可显著降低MoE部分的存储和访存开销。与INT8量化策略区分点：
- MoE：路由专家的Linear使用W4A8量化，共享专家的Linear使用W8A8量化；

>注：
W4A8：W4指权重使用静态Per-Channel Int4量化，A8指数据使用动态Per-Token Int8量化；
...
| ----------------- | ---------|----------  | ----- | ----------------- | ----------------- |----------------- |
| 16   | 16  | 3   | Hybrid MXFP8-MXFP4    | 8192     | 6.55      | 152.79  |
| 256  | 16  | 3   | Hybrid MXFP8-MXFP4    | 8192     | 8.78      | 1821.75 |
| 1536 | 16  | 1   | Hybrid MXFP8-MXFP4    | 8192     | 16.46     | 5830.65 |
| 2880 | 16  | 1   | Hybrid MXFP8-MXFP4    | 8192     | 25.54     | 7047.44 |
| 256  | 16  | 3   | Hybrid MXFP8-MXFP4    | 131072   | 9.66      | 1655.64 |
...
DeepSeek-V4 Pro Decode不同Batch Size和序列长度的性能Benchmark测试如下：
...
<!-- cann-meta
{
  "sidebarTitle": "大模型推理优化实践",
  "quantization": ["int8", "int4", "fp8", "mxfp8", "hif8", "mxfp4"],
  "parallelism": ["data-parallel", "tensor-parallel", "context-parallel", "expert-parallel"],
  "operator": ["tilelang", "pypto"],
  "cannFeatures": ["multi-stream", "superkernel"],
  "hardware": ["atlas-a3", "ascend-950"],
  "frameworks": ["cann-recipes"],
  "cover": {"type": "auto"},
  "llmSpeculativeInference": ["mtp"]
}
-->
